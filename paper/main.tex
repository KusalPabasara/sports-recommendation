...
\begin{itemize}
  \item We propose the first \textbf{interest-first sports recommendation
        ...
        secondary factors.
  \item We design a \textbf{stacking ensemble} combining hyperparameter-tuned
        XGBoost and Random Forest with a logistic regression meta-learner,
        achieving a 17.4\% NDCG improvement over the physical-only baseline
        using exclusively classical ML methods.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Work} & \textbf{Interest-first} & \textbf{Classical ML} & \textbf{Novice} & \textbf{Discovery} \\
\midrule
...
IARCI student \cite{iarci2023student}        & partial    & \checkmark & \checkmark & \texttimes \\
Ma et al.\ MTL \cite{ma2018mmoe}             & \texttimes & \texttimes & \texttimes & \texttimes \\
...
\texttt{subsample} $\in \{0.7, 0.8, 1.0\}$,
\texttt{colsample\_bytree} $\in \{0.7, 0.8, 1.0\}$.

\noindent\textbf{Random Forest:}
\texttt{n\_estimators} $\in \{100, 200, 300\}$,
\texttt{max\_depth} $\in \{5, 8, 12, \text{None}\}$,
\texttt{min\_samples\_leaf} $\in \{2, 5, 10\}$,
\texttt{max\_features} $\in \{\sqrt{d}, \log_2 d, 0.5d\}$.

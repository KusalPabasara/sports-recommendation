...
Novel-Sport Discovery Mechanism}
...
\begin{abstract}
% ============================================================
Existing sports matching systems rely on physical fitness metrics,
leaving personal interests underutilized despite evidence that
intrinsic motivation best predicts sustained engagement.
We present an \textit{interest-driven} recommendation framework
that positions personal interests as the primary signal, with physical
and demographic features as secondary factors.
Our system uses classical machine learning---hyperparameter-tuned
gradient boosting, random forests, and a stacking ensemble with
logistic regression as meta-learner---to predict playing and watching
preferences.
We additionally introduce a \textit{discovery score} based on
cosine similarity that surfaces unfamiliar sports aligned with
latent interests.
Evaluated on a literature-calibrated synthetic dataset of 1{,}000
users across 20~sports, the stacking ensemble achieves NDCG@5 of
0.498 (\textbf{17.4\% improvement} over a physical-only baseline).
SHAP attribution confirms that interest and self-rated strength
features account for 65.9\% of predictive importance, outweighing
objective physical metrics.
The discovery module achieves an 85.7\% novelty rate at $K{=}5$.
To our knowledge, this is the first framework that is simultaneously
interest-first, accessible to novice users, and equipped with a
novel-sport discovery mechanism---using only classical ML methods.
\end{abstract}

\begin{IEEEkeywords}
classical machine learning, discovery score, ensemble learning,
interest-driven, personalized recommendation, physical activity,
...
\begin{itemize}
  \item We propose the first \textbf{interest-first sports recommendation
        ...
        secondary factors.
  \item We design a \textbf{stacking ensemble} that combines tuned
        XGBoost and Random~Forest base learners with a logistic
        regression meta-learner, achieving 17.4\% NDCG improvement
        over the physical-only baseline using classical ML only.
  \item We introduce a \textbf{discovery score}, a cosine-similarity metric
        ...
        interest alignment---achieving a novelty rate of 85.7\% at $K{=}5$.
...
Critically, none of these systems model interest or motivation features, and the ceiling of
...
physical metrics as the primary signal and do not guide users toward
unfamiliar sports.
...
across a broad sports taxonomy and introduces no discovery mechanism.
...
Ensemble learning combines multiple base learners to improve
generalization~\cite{breiman2001random}.
Stacking~\cite{wolpert1992stacked} trains a meta-learner on
out-of-fold predictions from diverse base models, capturing
complementary strengths---e.g., gradient boosting for non-linear
interactions and bagging for variance reduction.
In recommendation, ensemble strategies are particularly effective
when feature spaces are heterogeneous~\cite{breiman2001random}.
We apply stacking with XGBoost and Random Forest as base learners
and logistic regression as meta-learner to the sports domain,
where interest, physical, and demographic features carry distinct
but complementary predictive signals.
SHAP-based feature attribution~\cite{lundberg2017shap} is used to verify
that interest features dominate predictions.
...
signal, (ii) targets general and novice users, (iii) surfaces
\textit{novel} sports via a discovery mechanism, and (iv) achieves
this using only classical ML methods without deep learning.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Work} & \textbf{Interest} & \textbf{Classical} & \textbf{Novice} & \textbf{Disc.} \\
\midrule
AI Sport Scout~\cite{aisportscout2024}    & \texttimes & \checkmark & \texttimes & \texttimes \\
Ramos et al.~\cite{ramos2022talent}       & \texttimes & \checkmark & \texttimes & \texttimes \\
Opast recs.~\cite{opast2023personalized}  & \texttimes & \checkmark & \checkmark & \texttimes \\
IARCI~\cite{iarci2023student}             & \textit{partial} & \checkmark & \checkmark & \texttimes \\
Ma et al.~\cite{ma2018mmoe}               & \texttimes & \texttimes & \texttimes & \texttimes \\
...
We define two binary label vectors per user:
$\mathbf{y}^{(\text{play})}, \mathbf{y}^{(\text{watch})} \in \{0,1\}^N$,
representing active participation and spectator interest, respectively.
The goal is to learn a function $f: \mathbb{R}^d \to [0,1]^{N}$ per task that
predicts the probability of engagement with each sport.
...
\begin{equation}
  y^{(\text{play})}_{u,s} \sim \text{Bernoulli}\!\left(\sigma\!\left(5(a_{u,s} - 1.5)\right)\right)
\end{equation}
where $\sigma$ is the logistic sigmoid, yielding $\approx 4$ sports
per user---consistent with participation
statistics~\cite{kokolakakis2016participation}.
...
\begin{figure}[t]
  ...
  in the synthetic dataset ($M = 1{,}000$). Football, cricket, and basketball
...
Default hyperparameters often underperform on novel tasks.
We apply grid search with 3-fold stratified cross-validation
to tune both XGBoost and Random Forest, searching over:

\smallskip
\noindent\textbf{XGBoost} search space:
\smallskip
...
\smallskip
\noindent\textbf{Random Forest} search space:
\smallskip

\noindent
\begin{tabular}{@{}ll@{}}
\texttt{n\_estimators}       & $\{100,\, 200,\, 300\}$ \\
\texttt{max\_depth}          & $\{5,\, 8,\, 12,\, \text{None}\}$ \\
\texttt{min\_samples\_leaf}  & $\{2,\, 5,\, 10\}$ \\
\texttt{max\_features}       & $\{\sqrt{d},\, \log_2 d,\, 0.5d\}$ \\
\end{tabular}
\smallskip

Tuning is performed on the first sport label (highest base rate) and the
selected hyperparameters are applied to all 20 per-sport classifiers via
\texttt{MultiOutputClassifier}, following the common approach for
multi-label problems with shared hyperparameters.

\subsection{Stacking Ensemble}

Our primary model is a stacking ensemble~\cite{wolpert1992stacked}
combining complementary base learners:

\begin{itemize}
  \item \textbf{Base learner 1}: XGBoost (\texttt{n\_estimators}=200,
        \texttt{max\_depth}=5, \texttt{lr}=0.05)
  \item \textbf{Base learner 2}: Random Forest (\texttt{n\_estimators}=200,
        \texttt{max\_depth}=8)
  \item \textbf{Meta-learner}: Logistic Regression (\texttt{max\_iter}=1000)
\end{itemize}

Base learners generate out-of-fold probability predictions via 3-fold CV,
which are concatenated and used as features for the logistic regression
meta-learner. This architecture leverages XGBoost's gradient boosting
for capturing non-linear feature interactions and Random Forest's bagging
for variance reduction, while the linear meta-learner provides calibrated
probability estimates and guards against overfitting.
The entire pipeline is wrapped in \texttt{MultiOutputClassifier} to
handle the 20-sport multi-label output.
...
Our stacking ensemble (S1) achieves the highest NDCG@5 among all
personalized models at 0.498, representing a \textbf{17.4\% improvement}
over the physical-only baseline (B3, NDCG@5 = 0.424) and a \textbf{16.1\%
improvement} over the untuned Full XGBoost (B5, NDCG@5 = 0.429).
The tuned Random Forest (T2, NDCG@5 = 0.493) achieves the highest MAP@5
(0.725), confirming that hyperparameter optimization yields meaningful gains.

The discovery-augmented variant (S2) achieves a \textbf{Discovery Rate@5 of
0.857}, surfacing novel sports for 85.7\% of top-5 slots, at the expected
cost of recommendation accuracy---a deliberate precision/novelty trade-off
controlled by the blending parameter $\alpha$.
...
Among personalized models, S1 achieves the best ranking quality (NDCG),
while S2 demonstrates that the discovery module can be activated to
...
\begin{figure}[t]
  ...
  The stacking ensemble (S1) achieves the highest NDCG among
  personalized models. The discovery-augmented variant (S2) maximizes
...
hyperparameter tuning, ensemble architecture, and the discovery module.
...
Combining all features with the stacking ensemble (S1) yields a
substantially higher NDCG of 0.498, demonstrating complementarity.

\noindent\textbf{Hyperparameter tuning.}
Tuned RF (0.493) marginally improves over the default RF (0.491) and
achieves the highest MAP@5 (0.725). Tuned XGBoost (0.413) underperforms
the default (0.429), suggesting the grid search overfits on the proxy
label; per-sport tuning is a direction for future work.

\noindent\textbf{Stacking ensemble.}
The stacking ensemble (S1, NDCG@5 = 0.498) outperforms all individual
models, confirming that combining XGBoost and RF through a logistic
regression meta-learner captures complementary patterns---gradient
boosting excels at non-linear interactions while bagging reduces variance.

\noindent\textbf{Discovery module.}
Enabling the discovery component (S2) shifts the trade-off decisively toward
novelty (Disc@5 = 0.857), showing the module effectively targets the
...
\begin{figure}[t]
  ...
  configurations. The stacking ensemble (S1) yields the best ranking
  quality; adding the discovery module (S2) trades accuracy for novelty.}
  \label{fig:ablation}
\end{figure}
...
The stacking ensemble again outperforms the untuned XGBoost baseline,
confirming that the ensemble approach generalizes across engagement levels.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Feature Group} & \textbf{Football} & \textbf{Swim.} & \textbf{Esports} & \textbf{Mean} \\
\midrule
...
Fig.~\ref{fig:shap_football} details the top individual feature
importances for the football prediction task.
...
The stacking ensemble consistently outperforms all individual models on
NDCG, confirming that combining gradient boosting (XGBoost) with bagging
(Random Forest) through a logistic regression meta-learner captures
complementary patterns in the feature space.
...
The discovery module (S2) demonstrates the intended behavior: its high
novelty rate (85.7\%) at the cost of lower accuracy is by design---the
module serves users who explicitly seek sports they have never tried.
...
sport discovery; (iv)~per-sport hyperparameter tuning and advanced
ensemble strategies such as Multi-gate Mixture-of-Experts~\cite{ma2018mmoe};
...
the key gaps identified in the literature: interest-first modeling,
accessibility to novice users, and discovery of unfamiliar sports---using
exclusively classical machine learning methods.
Our stacking ensemble achieves a 17.4\% NDCG improvement over
...
design principle.
The discovery module achieves an 85.7\% novelty rate, demonstrating
the viability of interest-similarity-based sport discovery.
We release our code, dataset, and trained models to
...
\smallskip
\noindent\textbf{Wandawa Gamage Kusal Pabasara}
is an undergraduate student in the Department of Computer Science
and Engineering at the University of Moratuwa, Sri Lanka.
His research interests include machine learning, recommender systems,
and data-driven approaches to sports analytics.
He is particularly interested in applying interpretable ML methods
to human-centered recommendation problems.
Contact: kusalp.23@cse.mrt.ac.lk.
